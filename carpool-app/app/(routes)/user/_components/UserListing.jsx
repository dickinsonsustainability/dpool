...
import { Calendar, Car, CircleDollarSign, Clock, MapPin, Trash, UserRound } from "lucide-react"
  ...
  return (
    <div className="my-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        {listing.length > 0 ? (
          listing.map((item) => (
            <div key={item.id} className="p-4 border rounded-lg shadow hover:border-primary cursor-pointer relative">
            <h2 className="bg-primary m-1 rounded-lg text-white absolute top-1 right-1 px-2 text-sm p-1">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2 mt-2 justify-between">
                  <div>
                    <TripDetail icon={MapPin} text={item?.departureAddress || 'N/A'} />
                    <TripDetail icon={MapPin} text={item?.arrivalAddress || 'N/A'} />
                  </div>
                  <div>
                    <TripDetail icon={Calendar} text={item?.date || 'No date'} />
                    <TripDetail icon={Clock} text={item?.time || 'No time'} />
                  </div>
                </div>
                <div className="flex gap-2 mt-2 justify-between">
                  <TripMeta icon={UserRound} text={item?.passenger || 'N/A'} />
                  <TripMeta icon={CircleDollarSign} text={typeof item?.price === "number" ? `$${item.price}` : item?.price || 'N/A'} />
